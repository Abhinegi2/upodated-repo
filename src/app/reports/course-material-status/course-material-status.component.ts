...
import { CourseMaterialStatusModel } from './course-material-status.model';
...
@Component({
  ...
  templateUrl: './course-material-status.component.html',
  styleUrls: ['./course-material-status.component.scss'],
  animations: fuseAnimations,
  encapsulation: ViewEncapsulation.None
})

export class CourseMaterialStatusComponent extends BaseListComponent<CourseMaterialStatusModel> implements OnInit {
  courseMaterialStatusForm: FormGroup;
  ...
  ngOnInit(): void {
    this.reportService.GetDropdownForReports(this.UserModel).subscribe(results => {
      ...
      if (currentYearItem != null) {
        ...
        this.courseMaterialStatusForm.get('AcademicYearId').setValue(this.currentAcademicYearId);

        if (this.UserModel.RoleCode === 'DivEO') {
          this.courseMaterialStatusForm.controls["DivisionId"].setValue(this.UserModel.DivisionId);


          this.onChangeDivision(this.UserModel.DivisionId).then(response => {
            this.getCourseMaterialStatusReports();
          });
        }
          ...
          this.onChangeDivision(this.UserModel.DivisionId).then(response => {
            ...
            this.courseMaterialStatusForm.controls["DistrictId"].setValue(districtIds);
    ...
    this.courseMaterialStatusForm = this.createCourseMaterialStatusForm();
  }

  resetReportFilters(): void {
    this.courseMaterialStatusForm.reset();
    this.courseMaterialStatusForm.get('AcademicYearId').setValue(this.currentAcademicYearId);
    ...
    let promise = new Promise((resolve, reject) => {
      ...
      this.commonService.GetMasterDataByType({ DataType: 'Districts', RoleId: this.UserModel.RoleCode, UserId: this.UserModel.UserTypeId, ParentId: divisionId, SelectTitle: 'District' }, false)
        .subscribe((response: any) => {
          ...
          this.courseMaterialStatusForm.controls['DistrictId'].setValue(null);
  ...
  createCourseMaterialStatusForm(): FormGroup {
  ...
  getCourseMaterialStatusReports(): void {

    if (!this.courseMaterialStatusForm.valid) {
      return;
    }
    ...
    this.reportService.GetCourseMaterialStatusReportsByCriteria(reportParams).subscribe(response => {
      this.displayedColumns = ['SrNo', 'AcademicYear', 'SchoolAllottedYear', 'PhaseName', 'VTPName', 'VCName', 'VCMobile', 'VCEmail', 'VTName', 'VTMobile', 'VTEmail', 'VTDateOfJoining', 'HMName', 'HMMobile', 'HMEmail', 'SchoolManagement', 'DivisionName', 'DistrictName', 'BlockName', 'UDISE', 'SchoolName', 'SectorName', 'JobRoleName', 'ClassName', 'CourceMaterialAvailability', 'DateOfReceipt','SubmissionDate', 'MonthYear', 'CourceMaterialStatus'];
