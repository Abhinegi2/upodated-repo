...
import { FieldAndIndustryVisitStatusModel } from './field-industry-visit-status.model';
...
@Component({
  ...
  templateUrl: './field-industry-visit-status.component.html',
  styleUrls: ['./field-industry-visit-status.component.scss'],
  animations: fuseAnimations,
  encapsulation: ViewEncapsulation.None
})

export class FieldAndIndustryVisitStatusComponent extends BaseListComponent<FieldAndIndustryVisitStatusModel> implements OnInit {
  fieldAndIndustryVisitStatusForm: FormGroup;
  ...
  ngOnInit(): void {
    this.reportService.GetDropdownForReports(this.UserModel).subscribe(results => {
      ...
      if (currentYearItem != null) {
        ...
        this.fieldAndIndustryVisitStatusForm.get('AcademicYearId').setValue(this.currentAcademicYearId);

        if (this.UserModel.RoleCode === 'DivEO') {
          this.fieldAndIndustryVisitStatusForm.controls["DivisionId"].setValue(this.UserModel.DivisionId);


          this.onChangeDivision(this.UserModel.DivisionId).then(response => {
            this.getFieldAndIndustryVisitStatusReports();
          });
        }
          ...
          this.onChangeDivision(this.UserModel.DivisionId).then(response => {
            ...
            this.fieldAndIndustryVisitStatusForm.controls["DistrictId"].setValue(districtIds);
    ...
    this.fieldAndIndustryVisitStatusForm = this.createFieldAndIndustryVisitStatusForm();
  }

  resetReportFilters(): void {
    this.fieldAndIndustryVisitStatusForm.reset();
    this.fieldAndIndustryVisitStatusForm.get('AcademicYearId').setValue(this.currentAcademicYearId);
    ...
    let promise = new Promise((resolve, reject) => {
      ...
      this.commonService.GetMasterDataByType({ DataType: 'Districts', RoleId: this.UserModel.RoleCode, UserId: this.UserModel.UserTypeId, ParentId: divisionId, SelectTitle: 'District' }, false)
        .subscribe((response: any) => {
          ...
          this.fieldAndIndustryVisitStatusForm.controls['DistrictId'].setValue(null);
  ...
  //Create FieldAndIndustryVisitStatus form and returns {FormGroup}
  createFieldAndIndustryVisitStatusForm(): FormGroup {
  ...
  getFieldAndIndustryVisitStatusReports(): void {
    if (!this.fieldAndIndustryVisitStatusForm.valid) {
      return;
    }
