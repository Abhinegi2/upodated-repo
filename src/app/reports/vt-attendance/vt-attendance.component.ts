...
import { VocationalTrainerAttendanceModel } from './vt-attendance.model';
...
@Component({
  ...
  templateUrl: './vt-attendance.component.html',
  styleUrls: ['./vt-attendance.component.scss'],
  animations: fuseAnimations,
  encapsulation: ViewEncapsulation.None
})

export class VocationalTrainerAttendanceComponent extends BaseListComponent<VocationalTrainerAttendanceModel> implements OnInit {
  vocationalTrainerAttendanceForm: FormGroup;
  ...
  ngOnInit(): void {
    this.reportService.GetDropdownForReports(this.UserModel).subscribe(results => {
      ...
      if (currentYearItem != null) {
        ...
        this.vocationalTrainerAttendanceForm.get('AcademicYearId').setValue(this.currentAcademicYearId);

        if (this.UserModel.RoleCode === 'DivEO') {
          this.vocationalTrainerAttendanceForm.controls["DivisionId"].setValue(this.UserModel.DivisionId);


          this.onChangeDivision(this.UserModel.DivisionId).then(response => {
            this.getVocationalTrainerAttendanceReports();
          });
        }
          ...
          this.onChangeDivision(this.UserModel.DivisionId).then(response => {
            ...
            this.vocationalTrainerAttendanceForm.controls["DistrictId"].setValue(districtIds);
    ...
    this.vocationalTrainerAttendanceForm = this.createVocationalTrainerAttendanceForm();
  }

  resetReportFilters(): void {
    this.vocationalTrainerAttendanceForm.reset();
    this.vocationalTrainerAttendanceForm.get('AcademicYearId').setValue(this.currentAcademicYearId);
    ...
    let promise = new Promise((resolve, reject) => {
      ...
      this.commonService.GetMasterDataByType({ DataType: 'Districts', RoleId: this.UserModel.RoleCode, UserId: this.Constants.DefaultStateId, ParentId: divisionId, SelectTitle: 'District' }, false)
        .subscribe((response: any) => {
          ...
          this.vocationalTrainerAttendanceForm.controls['DistrictId'].setValue(null);
  ...
  //Create VocationalTrainerAttendance form and returns {FormGroup}
  createVocationalTrainerAttendanceForm(): FormGroup {
  ...
  getVocationalTrainerAttendanceReports(): void {
    if (!this.vocationalTrainerAttendanceForm.valid) {
      return;
    }
    var reportParams = {
      UserId: this.UserModel.LoginId,
      AcademicYearId: this.vocationalTrainerAttendanceForm.get('AcademicYearId').value,
      DivisionId: this.vocationalTrainerAttendanceForm.get('DivisionId').value,
      DistrictId: this.vocationalTrainerAttendanceForm.get('DistrictId').value,
      SectorId: this.vocationalTrainerAttendanceForm.get('SectorId').value,
      JobRoleId: this.vocationalTrainerAttendanceForm.get('JobRoleId').value,
      VTPId: this.vocationalTrainerAttendanceForm.get('VTPId').value,
      ClassId: this.vocationalTrainerAttendanceForm.get('ClassId').value,
      MonthId: this.vocationalTrainerAttendanceForm.get('MonthId').value,
      SchoolManagementId: this.vocationalTrainerAttendanceForm.get('SchoolManagementId').value
    };
